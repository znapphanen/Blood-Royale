...
namespace BR.ExtraLib
{
    public class Security
    {
        ...
        //  static String KeyString = GenerateAPassKey("PassKey");
        //  static String EncryptedPassword = Encrypt("25Characterlengthpassword!", KeyString);
        //    String DecryptedPassword = Decrypt(EncryptedPassword, KeyString);

        private static string getKeyString()
        {
            string keyString = Sql.getPasskey();

            if (keyString.Equals(""))//there are no passkey
            {
                //generate a passkey and store it in db
                keyString = GenerateAPassKey("PassKey");
                Sql.setPasskey(keyString);
            }
        ...
        public static string EncryptPassword(string psw )
        {
            ...
            return Encrypt(psw, getKeyString());
        }

        public static string decryptPassword(string cryptedPsw)
        {
            return Decrypt(cryptedPsw, getKeyString());
        }
    }
}